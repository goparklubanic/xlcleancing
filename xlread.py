...
'''
def colfilter(df):
    df = df[['po#', 'item_no', 'metal', 'qty', 'total_wt', 'maklon', 'non_us_dia', 'total']]
    return df
...
def addtolist(df, itemlist_path):
    ...
    # loop the dataframe, append to result/itemlist.xlsx start from last row
    for i in range(len(df)):
        ws.cell(row=lastrow+i, column=1).value = df.iloc[i]['po#']
        ws.cell(row=lastrow+i, column=2).value = df.iloc[i]['item_no']
        ws.cell(row=lastrow+i, column=3).value = df.iloc[i]['metal']
        ws.cell(row=lastrow+i, column=4).value = df.iloc[i]['qty']
        ws.cell(row=lastrow+i, column=5).value = df.iloc[i]['total_wt']
        ws.cell(row=lastrow+i, column=6).value = df.iloc[i]['maklon']
        ws.cell(row=lastrow+i, column=7).value = df.iloc[i]['non_us_dia']
        ws.cell(row=lastrow+i, column=8).value = df.iloc[i]['total']
